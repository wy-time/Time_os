...
void HariMain(void)//主函数
{
	...
	vram = bootinfo->vram;
    unsigned char buf[32];//keyfifo的缓冲区
    unsigned char mouse_buf[128];//mousefifo的缓冲区
    char s[40];
    fifo8_init(&keyfifo,32,buf);//初始化缓冲区
    fifo8_init(&mousefifo,128,mouse_buf);//初始化缓冲区
	...
	{
        io_cli();//屏蔽所有中断
        if(keyfifo.free==keyfifo.size&&mousefifo.free==mousefifo.size)//缓冲区无内容
            io_stihlt();//开中断并等待
        else if(keyfifo.free!=keyfifo.size)
        {
        ...
        }else if(mousefifo.free!=mousefifo.size)
        {
            unsigned char data=fifo8_get(&mousefifo);
            io_sti();//开中断
            sprintf(s,"%02X",data);
            boxfill8(bootinfo->vram, bootinfo->scrnx, COL8_008484, 32, 16, 47, 31);
            putfonts8_asc(bootinfo->vram,bootinfo->scrnx,32,16,COL8_FFFFFF,(unsigned char*)s);
        }
	}
}
